...
こうする事で、少なくともレスポンスヘッダに関する処理と値はこのクラスにある、と言う事が分かり、コードの整理整頓に一役買っている。極論を言えばほとんどの構造、例えば`namespace`や、もうちょいミクロな粒度で言えば`function`や配列なんかも、<em>コードを整理する</em>、と言うのが主目的である。逆に言えば、プログラミングに置いて何がどこにあるか、と言うのは極めて重要なファクターと言えるワケだ。
