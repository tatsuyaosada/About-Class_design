...
プログラミングやその設計に置いてどんな場面でも「銀の弾丸」(何にでも使える万能な手法)は存在しない。今回解説するクラス、引いてはオブジェクト指向も同様で、極論すれば開発対象や他諸々の条件次第でケースバイケースと言うのが現実だ。故に「こんなもの使わなくても良くね？」と考えるのはある意味妥当とも言える。ただ、まがいなりにも30年程度の歴史を持ち、今なお様々な開発場面で用いられ続けている手法であるので、覚えておいて損はないと思う。多分。
