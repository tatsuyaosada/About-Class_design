...
それぞれ違う言語なので仕様や記述方法による多少の違いはあるが、大体こんな感じで似通った感じになる。と同時に、ここでクラスと言う構造を用いる最も重要な仕様であり、メリットの一つが言語化されている。上記のコードのインスタンス変数は両者とも`private`と言う修飾子が与えられている。このインスタンス変数は、このクラスの外部からは全く触れる事の出来ない状態になっていると言う事になり、要はクラス外部からは一切触る事が出来ないため、一般的な手続き型の変数と比べ、堅牢度が高い。
